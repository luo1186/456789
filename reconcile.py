...
核心对账引擎 v9
关联键：采购单号 + SKU
核心修复：dtype=str 读取，完整保留前导零（0019、0212等）
...
def _clean_sku(v) -> str:
    """
    SKU清洗，核心规则：
    - dtype=str读取后通常已正确，如 '0019' '0212' 'CP196'
    - 兼容仍有 .0 后缀的情况（CSV或旧格式），如 '2414.0' → '2414'
    - 绝不做数字转换，完整保留前导零
    """
    if v is None: return ""
    try:
...
def _reconcile(task_id, file_paths, result_dir, task_name):
    ...
    for _, srow in stmt_df.iterrows():
        ...
        if r_qty is None:
            errors.append(f"收货单未找到 采购单号[{po_no}]+SKU[{sku}]"); is_anomaly = True
...
def _write_excel(path, task_name, details, po_df, recv_df, stmt_df):
    ...
    headers    = ["采购单号", "SKU", "品名", "校验结果",
                  ...
                  "采购单单价", "单价来源", "对账单单价",
                  "行金额", "异常说明", "状态"]
    col_widths = [16, 10, 22, 16, 12, 12, 12, 10, 12, 12, 42, 10]
    ...
    for ri, d in enumerate(details, 2):
        ...
        if d["anomaly"] and d["note"] and d["note"] != "—":
            ...
            cmt = Comment(d["note"], "ReconCore")
            cmt.width = 320; cmt.height = 80
            nc.comment = cmt
